...
def main() -> int:
    ...
    parser.add_argument("--use-raghot-qa", action="store_true")
    ...
    rows = build_hybrid_sessions(
        ...
        raghot_rows=read_jsonl(Path(args.raghot)) if args.use_raghot_qa else None,
        use_raghot_qa=args.use_raghot_qa,
...
def select_next_hybrid_batch(
    ...
    *,
    raghot_rows: list[dict[str, Any]] | None,
    strict_counts: dict[tuple[str, str], int],
    longbench_exhaustion_report: dict[str, Any] | None,
    ...
    if required_per_cell <= 0:
        raise ValueError("required_per_cell must be positive")
    counts = _normalized_counts(strict_counts)
    ...
    use_raghot_qa = qa_needed and _demonstrates_longbench_qa_exhaustion(longbench_exhaustion_report)
    try:
        rows = build_hybrid_sessions(
            sharegpt_rows,
            longbench_rows,
...
def _normalized_counts(strict_counts: dict[tuple[str, str], int]) -> dict[tuple[str, str], int]:
    counts: dict[tuple[str, str], int] = {}
    for risk_family in RISK_FAMILIES:
        for task in SESSION_TASKS:
            count = int(strict_counts.get((risk_family, task), 0))
            if count < 0:
                raise ValueError(f"strict count must be non-negative: {risk_family}/{task}")
            counts[(risk_family, task)] = count
    return counts
...
def _demonstrates_longbench_qa_exhaustion(report: dict[str, Any] | None) -> bool:
    """Accept only the structured exhaustion status emitted by the LongBench batch scheduler."""
    return bool(
        isinstance(report, dict)
        and report.get("status") == "longbench_exhausted"
        and isinstance(report.get("counts"), dict)
    )
...
def _read_counts(path: Path) -> dict[tuple[str, str], int]:
    payload = json.loads(path.read_text(encoding="utf-8"))
    return {
        (risk_family, task): int(payload.get(f"{risk_family}/{task}", 0))
        for risk_family in RISK_FAMILIES
        for task in SESSION_TASKS
    }
...
def main() -> int:
    ...
    parser.add_argument("--longbench", default=str(LONG_BENCH_CANDIDATES))
    parser.add_argument("--raghot", default=str(RAGHOT_QA_CANDIDATES))
    parser.add_argument("--strict-counts", required=True)
    ...
    parser.add_argument("--longbench-exhaustion-report")
    ...
    batch = select_next_hybrid_batch(
        ...
        strict_counts=_read_counts(Path(args.strict_counts)),
        longbench_exhaustion_report=(
            json.loads(Path(args.longbench_exhaustion_report).read_text(encoding="utf-8"))
            if args.longbench_exhaustion_report
            else None
        ),
...
def test_next_hybrid_batch_is_twelve_sessions_sixty_requests_and_serial_async() -> None:
    batch = select_next_hybrid_batch(
        ...
        _longbench_rows(12),
        raghot_rows=_raghot_rows(12),
        strict_counts={(risk, task): 0 for risk in ("kivi_sensitive", "h2o_sensitive", "low_risk") for task in ("qa", "summary")},
        longbench_exhaustion_report={"status": "longbench_exhausted", "counts": {}},
...
def test_raghot_fallback_requires_longbench_exhaustion_report_and_underfilled_qa() -> None:
    strict_counts = {
        (risk, task): 0
        for risk in ("kivi_sensitive", "h2o_sensitive", "low_risk")
        for task in ("qa", "summary")
    }
    ...
    unverified = select_next_hybrid_batch(
        ...
        longbench_exhaustion_report=None,
        source_exhausted=False,
    )
