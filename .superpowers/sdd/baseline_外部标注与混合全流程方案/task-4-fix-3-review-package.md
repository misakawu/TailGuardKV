...
def build_hybrid_sessions(
    ...
    *,
    ...
    authoritative_longbench_rows: list[dict[str, Any]] | None = None,
    longbench_manifests: list[dict[str, Any]] | None = None,
    completed_longbench_prescreens: list[dict[str, Any]] | None = None,
    existing_candidate_rows: list[dict[str, Any]] | None = None,
    ...
    if use_raghot_qa and not raghot_qa_fallback_verified(
        authoritative_longbench_rows=authoritative_longbench_rows,
        longbench_manifests=longbench_manifests,
        completed_longbench_prescreens=completed_longbench_prescreens,
        existing_candidate_rows=existing_candidate_rows,
        measurements=measurements,
    ):
        raise ValueError("RAGhot QA requires a verified RAGhot fallback authorization")
...
def raghot_qa_fallback_verified(
    *,
    authoritative_longbench_rows: list[dict[str, Any]] | None,
    longbench_manifests: list[dict[str, Any]] | None,
    completed_longbench_prescreens: list[dict[str, Any]] | None,
    existing_candidate_rows: list[dict[str, Any]] | None,
    measurements: list[Any] | None,
    required_per_cell: int = 12,
) -> bool:
    """Require complete authoritative LongBench QA coverage and real final-form QA underfill."""
    if required_per_cell <= 0 or authoritative_longbench_rows is None:
        return False
    expected_qa_ids = [
        normalize_text(row.get("request_id"))
        for row in authoritative_longbench_rows
        if normalize_text(row.get("task")).lower() == "qa"
    ]
    if not expected_qa_ids or len(expected_qa_ids) != len(set(expected_qa_ids)):
        return False

    completed_by_id = {
        normalize_text(item.get("batch_id")): item for item in completed_longbench_prescreens or []
    }
    manifest_qa_ids: list[str] = []
    for manifest in longbench_manifests or []:
        manifest_candidates = list(manifest.get("candidates") or [])
        qa_ids = [
            normalize_text(candidate.get("request_id"))
            for candidate in manifest_candidates
            if normalize_text(candidate.get("task")).lower() == "qa"
        ]
        if not qa_ids:
            continue
        completed = completed_by_id.get(normalize_text(manifest.get("batch_id")))
        if completed is None:
            return False
        manifest_ids = {normalize_text(candidate.get("request_id")) for candidate in manifest_candidates}
        completed_ids = {
            normalize_text(candidate.get("request_id")) for candidate in completed.get("candidates", [])
        }
        ranked_ids = {
            normalize_text(candidate.get("request_id")) for candidate in completed.get("ranked_candidates", [])
        }
        if not manifest_ids or manifest_ids != completed_ids or manifest_ids != ranked_ids:
            return False
        manifest_qa_ids.extend(qa_ids)
    if len(manifest_qa_ids) != len(set(manifest_qa_ids)) or set(manifest_qa_ids) != set(expected_qa_ids):
        return False

    from label_sharegpt_sessions import strict_hybrid_cell_counts

    counts = strict_hybrid_cell_counts(existing_candidate_rows or [], measurements or [])
    return any(counts[(risk_family, "qa")] < required_per_cell for risk_family in ("kivi_sensitive", "h2o_sensitive", "low_risk"))
...
from build_hybrid_sessions import build_hybrid_sessions, raghot_qa_fallback_verified
from common import (
...
def select_next_hybrid_batch(
    ...
    *,
    ...
    authoritative_longbench_rows: list[dict[str, Any]],
    ...
    use_raghot_qa = qa_needed and raghot_qa_fallback_verified(
        authoritative_longbench_rows=authoritative_longbench_rows,
        longbench_manifests=longbench_manifests,
        completed_longbench_prescreens=completed_longbench_prescreens,
        existing_candidate_rows=existing_candidate_rows,
...
def main() -> int:
    ...
    parser.add_argument("--sharegpt", default=str(HYBRID_SKELETONS))
    parser.add_argument("--longbench", default=str(LONG_BENCH_CANDIDATES))
    parser.add_argument("--authoritative-longbench", default=str(LONG_BENCH_CANDIDATES))
    ...
    batch = select_next_hybrid_batch(
        ...
        authoritative_longbench_rows=read_jsonl(Path(args.authoritative_longbench)),
...
def test_raghot_builds_qa_only_with_session_consistent_content_provenance() -> None:
    ...
    rows = build_hybrid_sessions(
        ...
        use_raghot_qa=True,
        session_count=12,
        **_raghot_fallback_kwargs(longbench_rows),
    )
...
def _raghot_fallback_kwargs(longbench_rows: list[dict[str, object]]) -> dict[str, object]:
    manifests, completed = _longbench_completion_manifests(longbench_rows)
    return {
        "authoritative_longbench_rows": longbench_rows,
...
def test_final_export_rejects_raghot_summary_content() -> None:
    longbench_rows = _longbench_rows(72)
    rows = build_hybrid_sessions(
        _sharegpt_rows(72),
        longbench_rows,
...
def test_partial_manifest_inventory_cannot_authorise_raghot_fallback() -> None:
    authoritative_rows = _longbench_rows(24)
    pool_rows = authoritative_rows[:12]
    manifests, completed = _longbench_completion_manifests(pool_rows)
    batch = select_next_hybrid_batch(
        _sharegpt_rows(12),
        pool_rows,
        raghot_rows=_raghot_rows(12),
        existing_candidate_rows=[],
        measurements=[],
        authoritative_longbench_rows=authoritative_rows,
        longbench_manifests=manifests,
        completed_longbench_prescreens=completed,
        source_exhausted=False,
    )
    assert batch["status"] == "pending_measurement"
    assert batch["content_qa_source"] == "longbench"


def test_programmatic_builder_rejects_raghot_without_verified_fallback() -> None:
    with pytest.raises(ValueError, match="verified RAGhot fallback"):
        build_hybrid_sessions(
            _sharegpt_rows(12),
            _longbench_rows(12),
            raghot_rows=_raghot_rows(12),
