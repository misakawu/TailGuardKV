...
**目标：** 在 LongBench 已耗尽时，仅允许 KIVI-sensitive Summary 的直接预筛 reserve 从 12 降至 10，以继续构造并实测最终混合 session。

## 变更范围

- `kivi_sensitive/summary` 的 direct prescreen `required_per_cell` 为 10。
- 其余五个 `risk/task` direct prescreen cell 仍为 12。
...
2. 对五个未放宽 cell 要求至少 12 条；对 `kivi_sensitive/summary` 要求至少 10 条。
...
- 默认 gate 在现有计数 `kivi_sensitive/summary=10` 时返回可继续；9 时仍返回不足。
