...
- 已为 `transformers_profile_many_measurements()` 接通 `session_runtime` 与 `memory_budget_mib` 参数，并将 `session_runtime_state` / `memory_budget_mib` 透传到 `profiles.transformers_runtime` worker。
- 已让 `profiles.transformers_runtime.run_profile_batch()` 读写 `session_runtime_state`，并在 batch 路径中按 measured request 结果补齐 exact profile 的 `kv_incremental_mib`、`kv_cumulative_mib`、`resident_kv_mib_before`、`resident_kv_mib_after`、`restore_ms`、`recompute_ms`、`evicted_kv_mib`、`budget_hit`。
- 已拆分策略侧预算过滤与 backend 预算事件语义：
  - `Action` / `ActionDecision` 新增 `policy_budget_filtered`
  - `PolicyRunRecord` 新增 `policy_budget_filtered`、`backend_budget_hit`
  - `budget_hit` 在 `PolicyRunRecord` 中保留为 backend 事件别名，不再混合策略过滤信号
- 已更新 `run_util/run_policies.py` 与 `metrics/collector.py`，使 `budget_hit_rate` 仅统计 backend 事件，并新增 `policy_budget_filter_rate`。
- 已修正 policy projected memory 口径：
  - `StatsPolicy` 改为基于 replay-visible 的 session/global resident 状态投影
  - `FullLRUPolicy` 不再重复累加 `current_cumulative`
- 已保留并验证 baseline 的基本分化语义：
  - `static_best` 在 lossy 可部署时优先选择 lossy 固定 profile
  - `static_safe` 在无 safe lossy 时回退 exact，并保留 fallback 标记
  - `uncalibrated_dynamic` / `utility_dynamic` 继续使用 cache-state 感知的 memory filtering
- 已补充并通过相关回归测试：
  - `tests/test_transformers_session_runtime.py`
  - `tests/test_policy_session_budget.py`
  - `tests/test_session_summary_visuals.py`
  - `tests/test_session_profile_runtime.py`
  - `tests/test_tailguard_core.py` 中与 `transformers` / `full_lru` / baseline policy 相关子集

## 未完成验证

- `pilot-smoke-measured` 端到端重跑已启动尝试，但在本次会话内未拿到完成结果，因此当前尚无“5 个 baseline、2 个 budget、4 个 epsilon/delta 组合全部产出”的新鲜验证证据。
